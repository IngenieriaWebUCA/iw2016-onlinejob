...
privileged aspect EmpresaBatchService_Roo_GvNIXJpaBatch {
    ...
    public int EmpresaBatchService.deleteIn(List<Long> ids) {
        Query query = entityManager().createQuery("DELETE FROM Empresa as e WHERE e.id_ IN (:idList)");
    ...
    public int EmpresaBatchService.deleteNotIn(List<Long> ids) {
        Query query = entityManager().createQuery("DELETE FROM Empresa as e WHERE e.id_ NOT IN (:idList)");
        query.setParameter("idList", ids);
        return query.executeUpdate();
    }
